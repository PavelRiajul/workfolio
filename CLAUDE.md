...
- **The home scroll-stack stays sticky on mobile.** It only reads well once the cards are compact (they were 426px, which left the stack no room to show; ~380px works). Costs ~370px of page height — worth it, it's the signature interaction. Don't flatten it without looking at it first.
