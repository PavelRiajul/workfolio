...
- **`.work-thumb` is portrait (3:4) on desktop and reverts to 4:3 at ≤900px**, where the grid is 2-up on a phone and cards have to stay short. Covers are full-page screenshots — the live ones run past 7000px tall — so on hover the image **pans its own length**: it sits at natural height inside a `container-type: size` frame and slides up by `calc(100cqh - 100%)`, which is exactly its overflow. That expression is self-limiting, so it needs no per-image opt-in — an image no taller than the frame computes to 0 and doesn't move, and `min-height: 100%` keeps a short cover filling the frame as it always did. One fixed duration for every cover, so a very long page pans faster than a short one; killed under `prefers-reduced-motion`.
